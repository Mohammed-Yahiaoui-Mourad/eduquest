...
import { Calendar, FileSpreadsheet, FolderOpen, Plus, Trash2, Users, Info, Save, CheckCircle2, AlertCircle, Bell } from 'lucide-react';
    ...
    try {
      ...
      for (const [studentId, gradeData] of Object.entries(localGrades)) {
        ...
        const gradeToSave = existingGrade
          ? { ...existingGrade, ...gradeData }
          : { id: Date.now().toString() + studentId, studentId, moduleId: selectedModule, ...gradeData, approved: false };
...
export const TeacherResources = ({ teacherId }: { teacherId: string }) => {
  ...
  const addResource = async () => {
    setError('');
    if (!newRes.moduleId || !newRes.title || !newRes.url) {
    ...
    try {
      ...
      setDb(prev => ({ ...prev, resources: [...prev.resources, resourceData] }));
  ...
  return (
    <div className="space-y-6">
      ...
      {showAdd && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
                  <h3 className="text-2xl font-bold mb-6">Nouveau Support</h3>
                  <div className="space-y-4">
                      ...
                      <div>
                          <label className="block text-xs font-bold text-slate-400 uppercase mb-1">URL du fichier (Mock)</label>
                      ...
                      <div className="flex gap-4 pt-4">
                          <button onClick={() => setShowAdd(false)} className="flex-1 px-4 py-2 border rounded-xl font-bold">Annuler</button>
                          <button onClick={addResource} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold">Enregistrer</button>
                      </div>
                  </div>
              </div>
          </div>
