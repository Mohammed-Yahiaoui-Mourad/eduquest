                    ...
                    <div key={m.id} className={`p-4 bg-white border-2 ${borderClass} rounded-xl shadow-sm`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {getMissingFn(m).map(type => (
                          <span key={type} className={`px-2 py-1 ${bgClass} ${textClass} text-[10px] font-bold uppercase rounded border ${borderClass}`}>
                            {title.includes('EDT') ? 'Non programmé' : 'Sans Prof'}: {type}
                            {title.includes('Enseignants') && (
                              <button className="ml-2 px-2 py-1 bg-indigo-600 text-white rounded text-xs font-bold" onClick={() => setAssignModal({ module: m, type })}>
                                Assigner Prof
                              </button>
                            )}
                          </span>
...
export const StudentManager = ({ specialtyId }: { specialtyId: string }) => {
  ...
  const [activeLevel, setActiveLevel] = useState(levels[0] || '');
  const [activeSubSpec, setActiveSubSpec] = useState('');
  const [newStudent, setNewStudent] = useState({ fullName: '', username: '', subSpecialty: '', groupId: '' });
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  ...
  const addStudent = async () => {
    setError('');
    if (!newStudent.fullName || !newStudent.username || !activeLevel) {
      setError("Veuillez remplir tous les champs obligatoires (Nom et Username).");
      return;
    }
    ...
    try {
      ...
      setShowModal(false);
      setNewStudent({ fullName: '', username: '', subSpecialty: '', groupId: '' });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-indigo-700 shadow-sm">
          <Plus className="w-5 h-5" />
          Nouvel Enseignant
        </button>
      </div>
        ...
        <div className="lg:col-span-1 bg-slate-50 p-6 rounded-2xl border border-slate-200">
          ...
          <div className="space-y-4">
            <div>
              ...
              {assign.teacherId && (
                <p className="text-[10px] text-slate-500 mt-1">Enseignant actuellement affecté pour ce module/type</p>
              )}
            </div>
            <div>
